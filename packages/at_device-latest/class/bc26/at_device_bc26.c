...
static int bc26_power_on(struct at_device *device)
{
   ...
   //**** nead to set power_on ****//
		rt_pin_write(bc26->reset, PIN_HIGH);
		rt_thread_mdelay(200);
		rt_pin_write(bc26->reset, PIN_LOW);
    rt_pin_write(bc26->power_on, PIN_LOW);
...
int reset_nb_module(struct at_device *device)
{
  struct at_device_bc26 *bc26 = RT_NULL;
  bc26 = (struct at_device_bc26 *)device->user_data;
  rt_pin_write(bc26->reset, PIN_HIGH);
...
static int bc26_sleep(struct at_device *device)
{
    ...
    bc26->sleep_status = RT_TRUE;
		LOG_D("startup entry into sleep bc26->sleep_status %d\n",bc26->sleep_status);
...
static int bc26_wakeup(struct at_device *device)
{
    ...
    bc26->sleep_status = RT_FALSE;
		...
		LOG_D("wake up bc26->sleep_status.%d\n",bc26->sleep_status);
    at_delete_resp(resp);
    return(RT_EOK);
}
...
#define BC26_LINK_THREAD_STACK_SIZE     (1024 + 1024) //(1024 + 512)
...
static int bc26_netdev_set_up(struct netdev *netdev)
{
    ...
    if (device == RT_NULL)
    {
		...
		rt_kprintf("get device(%s) netdev->name test.", netdev->name);//add frank
...
static int bc26_init(struct at_device *device)
{
    ...
    bc26_socket_init(device);//2 frank
#endif
    ...
    //******** ctrl bc26 pwr_on reset init begin  *****//
		rt_pin_write(bc26->power_on, PIN_HIGH);
		rt_pin_mode(bc26->power_on, PIN_MODE_OUTPUT);
		rt_pin_write(bc26->reset, PIN_LOW);
		rt_pin_mode(bc26->reset, PIN_MODE_OUTPUT);
		rt_pin_write(bc26->psm_int, PIN_LOW);
		rt_pin_mode(bc26->psm_int, PIN_MODE_OUTPUT);
		//******** ctrl bc26 pwr_on reset init end  *****//
...
//********* ctrl device ops api *************// 
int ctrl_bc26_device_ops(const char *cmd)
{
    int result = RT_EOK;
    at_response_t resp = RT_NULL;
  //  struct at_device_bc26 *bc26 = RT_NULL;
    ...
    if (device == RT_NULL)
    {
        LOG_E("ser_ctrl get device(%s) failed.", "bc26");
        return -RT_ERROR;
    }
    resp = at_create_resp(64, 0, rt_tick_from_millisecond(300));
    if (resp == RT_NULL)
    {
		...
		if (rt_strncmp(cmd, "AT+QPOWD=0", strlen("AT+QPOWD=0")) == 0)
		{ 
		  result = bc26_power_off(device);
		  rt_kprintf("bc26_power_off comin %d!\n",result);
		} 
   else if(rt_strncmp(cmd, "sleep", strlen("sleep")) == 0)
	 { 
      result = bc26_sleep(device);
			rt_kprintf("bc26_sleep comin %d!\n",result);
	 
   	}
	 else if(rt_strncmp(cmd, "wakeup", strlen("wakeup")) == 0)
	 { 
	 
	    rt_kprintf("bc26_wakeup comin %d!\n",result);
      result = bc26_wakeup(device);
   	}
	 else if(rt_strncmp(cmd, "reset", strlen("reset")) == 0)
	 { 
	 
		  rt_kprintf("bc26_reset comin %d!\n",result);
      result = reset_nb_module(device);
   	}
...
#endif  //class->device_ops->init(device); 
