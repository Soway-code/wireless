...
#include <stm32h7xx_hal.h> //frank
#include <Swy_nbapp.h>  

#define LOG_TAG                        "at.skt.bc26"
#include <at_log.h>
extern Swy_mqtt_info_t _mq_conn;
...
int mqtt_on_flag=0;
extern Swy_tcp_info_t _tcp_conn;//add frank
...
{
   rt_kprintf("rt_event_send  %d \n",rt_event_send(device->socket_event, event));
    ...
    enum at_socket_type type, rt_bool_t is_client)
{
    ...
    for(i=0; i<CONN_RETRY; i++)//CONN_RETRY
    {
        /* clear socket connect event */
        event = SET_EVENT(device_socket, BC26_EVENT_CONN_OK | BC26_EVENT_CONN_FAIL);
       event_result = bc26_socket_event_recv(device, event, 0, RT_EVENT_FLAG_OR);
		#if ((Swy_tcp == 1) || (Swy_udp == 1))  
				if (at_obj_exec_cmd(device->client, resp, "AT+QIOPEN=1,%d,\"%s\",\"%s\",%d,0,1",device_socket, type_str,ip, port) < 0) 
				{
		...
		#elif (Swy_mqtt == 1)
				 if (_mq_conn.mqtt_open != 1)
					 {
						 if (at_obj_exec_cmd(device->client, resp, "AT+QMTOPEN=0,\"%s\",%d",ip, port) < 0) 
						 {
								LOG_E("check_QMTCONN_fail\n");
								result = -RT_ERROR;
								break;
							}
						 HAL_Delay(1500U);//rt_thread_mdelay(1000);
				 	}
				 
				 if (_mq_conn.mqtt_conn != 1)
					 {
						 if (at_obj_exec_cmd(device->client, resp, "AT+QMTCONN=0,\"%s\",\"%s\",\"%s\"","866971030521988","866971030521988","e938e6daf27427554a01022333b8c5c8") < 0) 
							 {
									LOG_E("check_QMTOPEN_fail\n");
									result = -RT_ERROR;
									break;
							 }
					 	  HAL_Delay(1200U);// rt_thread_mdelay(1000);
				 	}
				 
				 if (_mq_conn.mqtt_sub != 1)
					 {
						 if (at_obj_exec_cmd(device->client, resp, "AT+QMTSUB=0,1,\"%s\",%d","quec/866971030521988/down", 2) < 0) 
						 	{
						 	 LOG_E("check_QMTSUB_fail\n");
						 	 result = -RT_ERROR;
							 break;
						 	}
						 HAL_Delay(1200U);//rt_thread_mdelay(1000);
        ...
        event_result = bc26_socket_event_recv(device, SET_EVENT(device_socket, 0), 
                                    60 * RT_TICK_PER_SECOND, RT_EVENT_FLAG_OR);
				rt_kprintf("bc26_socket_connect_envent_recv2 %d %d \n",event_result,SET_EVENT(device_socket, 0));//trace frank
        ...
        event_result = bc26_socket_event_recv(device, BC26_EVENT_CONN_OK | BC26_EVENT_CONN_FAIL, 
				...
				rt_kprintf("bc26_socket_connect_envent_recv3 %d %d \n",event_result,(BC26_EVENT_CONN_OK | BC26_EVENT_CONN_FAIL));//trace frank
...
static int bc26_socket_send(struct at_socket *socket, const char *buff, size_t bfsz, enum at_socket_type type)
{
    ...
    while (sent_size < bfsz)
    {
        ...
        /* send the "AT+QISEND" commands to AT server than receive the '>' response on the first line. */			
				...
				if (at_obj_exec_cmd(device->client, resp, "AT+QISEND=%d,%d", device_socket, (int)cur_pkt_size) < 0)
					{ 
					rt_kprintf("at_bc26_fail");
							result = -RT_ERROR;
							goto __exit;
					}

		#elif (Swy_mqtt == 1)
				if (at_obj_exec_cmd(device->client, resp, "AT+QMTPUB=%d,%d,%d,%d,\"%s\"",0,0,0,0,"quec/866971030521988/up") < 0)
					{
					rt_kprintf("at_bc26_mqfail");
							result = -RT_ERROR;
							goto __exit;
					}

		#endif
        rt_thread_mdelay(20);//delay at least 4ms
    ...
    #endif
        if ((type == AT_SOCKET_TCP) && (Swy_mqtt != 1))
        {
            ret=at_wait_send_finish(socket, 2*cur_pkt_size);
        }
...
static void urc_connect_func(struct at_client *client, const char *data, rt_size_t size)
{
    ...
    if (result == 0)
    {
      result = bc26_socket_event_send(device, SET_EVENT(device_socket, BC26_EVENT_CONN_OK));
       rt_kprintf("urc_connect_func_envent_send %d %d \n",result,SET_EVENT(device_socket, BC26_EVENT_CONN_OK));//trace frank
		}
    else
    {
        ...
        bc26_socket_event_send(device, SET_EVENT(device_socket, BC26_EVENT_CONN_FAIL));
				...
				rt_kprintf("urc_connect_func_envent_send_fail %d %d \n",result,SET_EVENT(device_socket, BC26_EVENT_CONN_FAIL));//trace frank
		}
}
...
static void urc_close_func(struct at_client *client, const char *data, rt_size_t size)
{
    ...
    struct at_socket *socket = RT_NULL;
		struct at_socket *sock = RT_NULL; //add frank
    struct at_device *device = RT_NULL;
    char *client_name = client->device->parent.name;
    ...
    _tcp_conn.state = AT_SOCKET_CLOSED;
		_mq_conn.state = AT_SOCKET_CLOSED;
...
//******  enfuc *****//
/*
#define SW_SAL_INTERNET_IP   "118.31.15.152"  // frank
#define SAL_INTERNET_PORT    8101  

//从NB模块接收数据接口 
int SWYCheckMsgDataContentLength(const char *readBuf,rt_size_t readLen)
{
    rt_size_t bufLen=256;
		if(readLen>bufLen || readLen > readLen || readLen<2)
			return 0;
		else if((readBuf[1]==0)&&(readLen==2))
			return 2;
		else if(readBuf[1]!=0){
		
		
		}
		rt_kprintf("parse&logical_go...%s \n",readBuf);
